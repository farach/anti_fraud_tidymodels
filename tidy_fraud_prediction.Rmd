...
Supervised Machine Learning for Text Analysis in R: https://smltar.com/preface.html
...
```{r}
yardstick::accuracy(fraud_results, truth = isFraud, estimate = .pred_class)
```
...
```{r}
yardstick::precision(fraud_results, truth = isFraud, estimate = .pred_class)
```
...
```{r}
yardstick::recall(fraud_results, truth = isFraud, estimate = .pred_class)
```
...
```{r}
yardstick::specificity(fraud_results, truth = isFraud, estimate = .pred_class)
```
...
```{r}
yardstick::f_meas(fraud_results, truth = isFraud, estimate = .pred_class)
```
...
fraud_final_fit %>% 
  ...
  roc_curve(truth = isFraud, estimate = .pred_class) %>%
  # Plot the ROC curve
  autoplot() +
  labs(
    x = "1-specificity\n(proportion of false positives\namong true negatives)",
...
# on incoming data.
preds_res <- predict(fraud_wf_model, sample_n(fraud_data, 1), type = "class")
